...
<style>
  ...
  .meta-table { width: 100%; margin-bottom: 24px; background: #FAFAF9; border-radius: 6px; padding: 12px 14px; }
  ...
  <tr>
    ...
    <td>
      <table class="meta-table">
        ...
        <tr><td class="meta-label">Referentie</td><td class="meta-value">{{ $invoice->reference }}</td></tr>
        @endif
        @if($company->kvk_number)
        <tr><td class="meta-label">KVK</td><td class="meta-value">{{ $company->kvk_number }}</td></tr>
        @endif
        @if($company->vat_number)
        <tr><td class="meta-label">BTW</td><td class="meta-value">{{ $company->vat_number }}</td></tr>
        @endif
      </table>
    </td>
  </tr>
...
<table class="totals">
  ...
  @if(is_array($invoice->vat_breakdown))
    @foreach($invoice->vat_breakdown as $rate => $amount)
      <tr>
        <td class="label">BTW {{ rtrim(rtrim(number_format((float) $rate, 2, ',', '.'), '0'), ',') }}%</td>
        <td class="value">€ {{ number_format((float) $amount, 2, ',', '.') }}</td>
      </tr>
    @endforeach
  @endif
  <tr class="grand-row">
    <td>Te betalen</td>
    <td class="value brand">€ {{ number_format($invoice->total, 2, ',', '.') }}</td>
  </tr>
</table>
...
@if($company->iban)
<div class="notes">
  Gelieve het bedrag binnen <strong>{{ $invoice->payment_terms }} dagen</strong> over te maken naar
  <strong>{{ $company->iban }}</strong> ten name van <strong>{{ $company->name }}</strong>
  @if($invoice->number) onder vermelding van factuurnummer <strong>{{ $invoice->number }}</strong>@endif.
</div>
@endif

@if($invoice->footer)
<div class="footer">{!! nl2br(e($invoice->footer)) !!}</div>
@elseif($company->invoice_footer)
<div class="footer">{!! nl2br(e($company->invoice_footer)) !!}</div>
@endif
