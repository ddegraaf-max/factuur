...
<head>
    ...
    <title>{{ __('Toegangscode klantenportaal') }}</title>
    <style>
            ...
            <div class="header">
                <div class="logo">
                    <table role="presentation" cellpadding="0" cellspacing="0"><tr>
                        ...
                        <td><div class="name">{{ brand('name') }}</div><div class="tag">🔒 {{ __('Klantenportaal · beveiligde omgeving') }}</div></td>
                    </tr></table>
                </div>
            </div>
            <div class="body">
                <h1>{{ __('Je toegangscode') }}</h1>
                <p>{{ __('Met onderstaande code krijg je veilig toegang tot je facturen en offertes in het online klantenportaal.') }}</p>

                <div class="code-box">
                    <div class="code-label">{{ __('Toegangscode') }}</div>
                    <div class="code">{{ $code }}</div>
                </div>

                <p>{!! __('De code is <strong>10 minuten</strong> geldig en werkt alleen in het browservenster waarin je hem hebt aangevraagd.') !!}</p>

                <div class="meta">
                    {{ __('Heb jij dit niet aangevraagd? Dan kun je deze e-mail veilig negeren — zonder deze code krijgt niemand toegang tot je documenten.') }}
                </div>
            </div>
            <div class="footer">
                © {{ date('Y') }} {{ brand('name') }} · {{ __('Beveiligd klantenportaal voor facturen en offertes') }}
            </div>
        </div>
    </div>
</body>
