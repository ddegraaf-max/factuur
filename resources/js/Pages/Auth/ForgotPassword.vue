...
<template>
  ...
  <AuthLayout>
    <template #hero>
      <h2>{{ $t('Wachtwoord vergeten?') }}</h2>
      <p>{{ $t('Geen probleem. Vul je e-mailadres in en we sturen je een link om een nieuw wachtwoord in te stellen.') }}</p>
    </template>

    <div class="login-form-card">
      <div class="login-form-title">{{ $t('Wachtwoord vergeten') }}</div>
      <div class="login-form-sub">{{ $t('We mailen je een herstel-link') }}</div>
      ...
      <form @submit.prevent="submit">
        ...
        <button class="btn btn-primary btn-block" type="submit" :disabled="form.processing">
          {{ form.processing ? $t('Bezig…') : $t('Stuur herstel-link') }}
        </button>
      </form>
